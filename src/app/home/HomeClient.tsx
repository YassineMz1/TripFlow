...
const FEATURES: Feature[] = [
  { title: "Smart Itineraries", subtitle: "AI-crafted plans in minutes", icon: "🧭", gradient: "from-[#3A67FF] to-[#2CC7A5]" },
  { title: "Live Collaboration", subtitle: "Plan trips together in real-time", icon: "🤝", gradient: "from-[#3F7BFA] to-[#72D5FF]" },
  { title: "Map + Timeline", subtitle: "Visualize routes and schedules", icon: "🗺️", gradient: "from-[#2FB46B] to-[#77DD77]" },
  ...
  const t = (k: string) => {
    const dict: Record<string, Record<string, string>> = {
      en: {
        ...
        heroSub: "Build itineraries in minutes, coordinate with friends, and explore curated experiences — all in one place.",
        whereTo: "Where to?",
      ...
      fr: {
        ...
        heroSub: "Crée des itinéraires en quelques minutes, coordonne-toi avec tes amis et explore des expériences sélectionnées — tout au même endroit.",
        whereTo: "Où aller ?",
      ...
      es: {
        ...
        heroSub: "Crea itinerarios en minutos, coordínate con amigos y explora experiencias seleccionadas, todo en un solo lugar.",
        whereTo: "¿A dónde?",
      ...
      de: {
        ...
        heroSub: "Erstelle in Minuten Reisepläne, koordiniere dich mit Freunden und entdecke kuratierte Erlebnisse – alles an einem Ort.",
        whereTo: "Wohin?",
  ...
  return (
    <main className="min-h-dvh" style={{ background: "var(--background)", color: "var(--foreground)" }}>
      ...
      <section className="max-w-7xl mx-auto px-4 pt-16 pb-12 relative">
        <div className="pointer-events-none absolute -top-16 -right-24 h-72 w-72 rounded-full blur-3xl opacity-30 hidden sm:block" style={{ background: "radial-gradient(closest-side, #7C4DFF, transparent)" }} />
        <div className="pointer-events-none absolute -bottom-16 -left-24 h-72 w-72 rounded-full blur-3xl opacity-30 hidden sm:block" style={{ background: "radial-gradient(closest-side, #2CC7A5, transparent)" }} />
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
          <div>
            <h1 className="text-4xl sm:text-5xl font-extrabold leading-tight">{t("heroTitle")}</h1>
            <p className="mt-4 text-base sm:text-lg" style={{ color: "var(--muted-foreground)" }}>{t("heroSub")}</p>

            {/* Search / Planner quick start */}
            <div className="mt-8 rounded-2xl p-4 sm:p-5 backdrop-blur" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
      ...
      <section className="max-w-7xl mx-auto px-4 py-12">
        ...
        <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {FEATURES.map((f, i) => (
            <div key={i} className="relative rounded-3xl p-5 text-white" style={{ boxShadow: "0 20px 60px rgba(0,0,0,0.20)" }}>
      ...
      {/* Trust bar / CTA */}
      <section className="max-w-7xl mx-auto px-4 py-12">
        <div className="rounded-3xl p-6 sm:p-8 flex flex-col sm:flex-row items-center justify-between gap-5" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
          <div>
          ...
          <div className="flex items-center gap-3">
            <Link href="" className="rounded-xl px-4 py-3 font-semibold" style={{ background: "#2563eb", color: "#fff" }}>{t("getStarted")}</Link>
            <Link href="/home" className="rounded-xl px-4 py-3 font-semibold" style={{ border: "1px solid var(--border)" }}>{t("explore")}</Link>
          </div>
        </div>
      </section>
